import os
##计算学分
def calnum(fen,ji,num,classji,chong):
...
##计算学分绩
def caltotal(fen,ji,num,classji,chong):
    if classji in ['EX','P','W','I','G']:
        return num
    elif classji == 'F':
        if ji == 'N/A':
    ...
    with open('__temp.txt','r')as inp,open(shuchu_name,'a') as summary:
        ...
        for i in chengji_total:
            ...
            if i[shuxing] == '必修':
                ...
                bixiu_num[poc] = calnum(i[xuefen],i[chengji],bixiu_num[poc],i[chengji_class],i[chongxiu])
                bixiu_total[poc] = caltotal(i[xuefen],i[chengji],bixiu_total[poc],i[chengji_class],i[chongxiu])
            elif i[shuxing] == '限选':
                if i[chengji_class] not in['F','W','I']:
                    xuefen_total[poc] = xuefen_total[poc]+int(i[xuefen])
                xianxuan_num[poc] = calnum(i[xuefen],i[chengji],xianxuan_num[poc],i[chengji_class],i[chongxiu])
                xianxuan_total[poc] = caltotal(i[xuefen],i[chengji],xianxuan_total[poc],i[chengji_class],i[chongxiu])
            else:
                if i[chengji_class] not in['F','W','I']:
                    xuefen_total[poc] = xuefen_total[poc]+int(i[xuefen])
                renxuan_num[poc] = calnum(i[xuefen],i[chengji],renxuan_num[poc],i[chengji_class],i[chongxiu])
                renxuan_total[poc] = caltotal(i[xuefen],i[chengji],renxuan_total[poc],i[chengji_class],i[chongxiu])

        print("总统计人数为："+str(j)+"\n")
    
        summary.write('学号\t姓名\t教学班级\t总完成学分\t必修限选平均学分绩\t必修限选平均学分绩排名\t所有课程平均学分绩\t所有成绩平均学分绩排名\t总学分绩\t总学分绩排名\n')
        for g in range(0,j):
        ...
        for g in range(0,j):
            summary.write(str(num_n[g])+"\t"+str(name_n[g])+"\t"+str(classname_n[g])+"\t"+str(xuefen_total[g])+"\t"+str(round(xiuxian_aver[g],3))+'\t'+str(sorted(xiuxian_aver,reverse = True).index(xiuxian_aver[g])+1)+'\t'+str(round(all_aver[g],3))+'\t'+str(sorted(all_aver,reverse = True).index(all_aver[g])+1)+'\t'+str(round(all_add[g],3))+'\t'+str(sorted(all_add,reverse = True).index(all_add[g])+1)+'\n')
    os.remove('__temp.txt') 

print("亲爱的用户，欢迎您试用“李导帮你算成绩”程序2.0版本\n感谢您对本程序的支持\n如有程序运行错误，欢迎微信联系作者李天奇：litq_94\n一键生成成绩排名-尽享假期生活\n\n")
wenjian_name = input("请输入你要进行成绩排序的文件（需要同时输入文件后缀），回车键结束\n")
shuchu_name = input("请输入你想要写入的文件名称（为方便打开，建议您同时输入文件后缀），回车键结束\n")
data_test(wenjian_name)
mainprocess(shuchu_name)
